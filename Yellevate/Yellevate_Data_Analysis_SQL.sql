...
WITH stats AS (
    ...
    JOIN
        pg_class
    ON
        pg_stats.tablename = pg_class.relname AND
    ...
    JOIN
        information_schema.columns
    ON
        information_schema.columns.column_name = pg_stats.attname AND
        information_schema.columns.table_name = pg_stats.tablename
    WHERE
        tablename = 'invoices'
    AND
        n_distinct != -1
